...
def run_pipeline() -> None:
    ...
    # generate_eda_artifacts is removed since plots are dynamic in the dashboard.
    ...
    # Apply log1p transformation to avoid negative predictions and handle skewness
    X_train = df_train_engineered[features]
    y_train = np.log1p(df_train_engineered['revenue'])
    
    X_test = df_test_engineered[features]
    y_test = np.log1p(df_test_engineered['revenue'])
